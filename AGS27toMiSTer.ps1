...
FileRequired -path "$($AGS27hdf)\Workbench.hdf"
FileRequired -path "$($AGS27hdf)\AGS_Drive.hdf"
FileRequired -path "$($AGS27hdf)\Games.hdf"
FileRequired -path "$($AGS27hdf)\Extra.hdf"
FileRequired -path "$($AGS27hdf)\Work.hdf"
FileRequired -path "$($AGS27hdf)\Media.hdf"

<#
##################################################
# Create image
$AGS27MiSTerImage = "$($BuildHDD)AGS27MiSTer.hdf"
...
& $HSTImagerExe fs copy "$($HDFSource)\rdb\dh4" "$($AGS27MiSTerImage)\rdb\dh4" --recursive --makedir --force --verbose
# Copy Media
$HDFSource = "$($AGS27hdf)\Media.hdf"
& $HSTImagerExe fs copy "$($HDFSource)\rdb\dh5" "$($AGS27MiSTerImage)\rdb\dh5" --recursive --makedir --force --verbose
...
$AGS27MiSTerPrimary = "$($BuildHDD)AGS27MiSTer_Primary.hdf"
# Primary image Size 25GB + 2.16MB (Partition table)
$AGS27MiSTerPrimarySize = (1024 * 1024 * 1024 * 25) + (1024 * 1024 * 2.16)

if (Test-Path $AGS27MiSTerPrimary) {
	Write-Host "Old image $($AGS27MiSTerPrimary) exists, removing."
	$null = Remove-Item -Path $AGS27MiSTerPrimary
}
Write-Host "Creating image $($AGS27MiSTerPrimary), $($AGS27MiSTerPrimarySize) bytes."

# create blank image
& $HSTImagerExe blank $AGS27MiSTerPrimary $AGS27MiSTerPrimarySize --verbose
# initialize rigid disk block
...
# add rdb partition of 16gb space with device name "DH1" "AGS_Drive"
& $HSTImagerExe rdb part add $AGS27MiSTerPrimary DH1 PDS3 16gb --verbose
# add rdb partition of 8gb space with device name "DH2" "Games"
& $HSTImagerExe rdb part add $AGS27MiSTerPrimary DH2 PDS3 8gb --verbose

# format rdb partition number 1 with volume name "Workbench"
& $HSTImagerExe rdb part format $AGS27MiSTerPrimary 1 Workbench --verbose
# format rdb partition number 2 with volume name "AGS_Drive"
& $HSTImagerExe rdb part format $AGS27MiSTerPrimary 2 AGS_Drive --verbose
# format rdb partition number 3 with volume name "Games"
& $HSTImagerExe rdb part format $AGS27MiSTerPrimary 3 Games --verbose


##################################################
# Create secondary image
$AGS27MiSTerSecondary = "$($BuildHDD)AGS27MiSTer_Secondary.hdf"
# Primary image Size 30GB + 2.38MB (Partition table)
$AGS27MiSTerSecondarySize = (1024 * 1024 * 1024 * 30) + (1024 * 1024 * 2.38)

if (Test-Path $AGS27MiSTerSecondary) {
	Write-Host "Old image $($AGS27MiSTerSecondary) exists, removing."
	$null = Remove-Item -Path $AGS27MiSTerSecondary
}
Write-Host "Creating image $($AGS27MiSTerSecondary), $($AGS27MiSTerSecondarySize) bytes."

# create blank image
& $HSTImagerExe blank $AGS27MiSTerSecondary $AGS27MiSTerSecondarySize --verbose
# initialize rigid disk block
...
# add rdb partition of 4gb space with device name "DH4" "Work"
& $HSTImagerExe rdb part add $AGS27MiSTerSecondary DH4 PDS3 4gb --verbose
# add rdb partition of 10gb space with device name "DH5" "Media"
& $HSTImagerExe rdb part add $AGS27MiSTerSecondary DH5 PDS3 10gb --verbose

# format rdb partition number 1 with volume name "Extra"
& $HSTImagerExe rdb part format $AGS27MiSTerSecondary 1 Extra --verbose
# format rdb partition number 2 with volume name "Work"
& $HSTImagerExe rdb part format $AGS27MiSTerSecondary 2 Work --verbose
# format rdb partition number 3 with volume name "Media"
& $HSTImagerExe rdb part format $AGS27MiSTerSecondary 3 Media --verbose

##################################################
# Copy Workbench
$HDFSource = "$($AGS27hdf)\Workbench.hdf"
& $HSTImagerExe fs copy "$($HDFSource)\rdb\dh0" "$($AGS27MiSTerPrimary)\rdb\dh0" --recursive --makedir --force --verbose
# Copy AGS_Drive
$HDFSource = "$($AGS27hdf)\AGS_Drive.hdf"
& $HSTImagerExe fs copy "$($HDFSource)\rdb\dh1" "$($AGS27MiSTerPrimary)\rdb\dh1" --recursive --makedir --force --verbose
# Copy Games
$HDFSource = "$($AGS27hdf)\Games.hdf"
& $HSTImagerExe fs copy "$($HDFSource)\rdb\dh2" "$($AGS27MiSTerPrimary)\rdb\dh2" --recursive --makedir --force --verbose
# Copy Extra
$HDFSource = "$($AGS27hdf)\Extra.hdf"
& $HSTImagerExe fs copy "$($HDFSource)\rdb\dh3" "$($AGS27MiSTerSecondary)\rdb\dh3" --recursive --makedir --force --verbose
# Copy Work
$HDFSource = "$($AGS27hdf)\Work.hdf"
& $HSTImagerExe fs copy "$($HDFSource)\rdb\dh4" "$($AGS27MiSTerSecondary)\rdb\dh4" --recursive --makedir --force --verbose
# Copy Media
$HDFSource = "$($AGS27hdf)\Media.hdf"
& $HSTImagerExe fs copy "$($HDFSource)\rdb\dh5" "$($AGS27MiSTerSecondary)\rdb\dh5" --recursive --makedir --force --verbose


##################################################
# show info
& $HSTImagerExe info $AGS27MiSTerPrimary
& $HSTImagerExe info $AGS27MiSTerSecondary
...
& $HSTImagerExe fs extract "$($BuildFolder)MiSTer_share.lha\DEVS\dummy.device" "$($AGS27MiSTerImage)\rdb\dh0\Devs\" --force --verbose
& $HSTImagerExe fs extract "$($BuildFolder)MiSTer_share.lha\DEVS\DOSDrivers\SHARE*" "$($AGS27MiSTerImage)\rdb\dh0\Devs\DOSDrivers\" --makedir --force --verbose
& $HSTImagerExe fs extract "$($BuildFolder)MiSTer_share.lha\L\*" "$($AGS27MiSTerImage)\rdb\dh0\L\" --makedir --force --verbose
...
& $HSTImagerExe fs extract "$($BuildFolder)MiSTer_RTG.lha" "$($AGS27MiSTerImage)\rdb\dh0\" --recursive --makedir --force --verbose
